...
- 每个标题最多 22 个中文字，**但长度必须参差不齐**：至少有 2 个是 5-8 字的短标题，2 个是 15+ 字的长标题，剩下的中等长度。绝对不要 ${count} 个都差不多长。
- **切入角度必须多样**，不要 ${count} 条都是同一个套路。混着用下面这些形式，每种最多出现 2 次：
  • 一个具体人物/场景（外卖小哥的破本子）
  • 一个数字或反常识事实（1400 万人在做这件事）
  • 一个疑问句（为什么便利店关东煮总在下午卖光）
  • 一句反直觉断言（越便宜的咖啡豆越难做拿铁）
  • 一个对比（十年老板 vs 三个月新手）
  • 一句大白话钩子（那年我在县城开了家书店）
  • 一个具体地点/时间锚点（凌晨四点的菜市场）
- 禁止抽象词：心态、格局、底层逻辑、本质、认知、思考、启示、感悟、价值、意义、重要性、不可或缺、提升自我。
- 禁止"我的思考"、"关于 XX"、"聊聊 XX"、"XX 的重要性"这种万能标题。
- 不要 emoji、书名号、Markdown。允许问号，其他标点尽量少。
...
输出格式：直接输出 ${count} 行，每行一个标题，标题之间只有换行。不要编号、不要项目符号、不要 Markdown、不要任何其他前后文字或说明。`;
  ...
  for (const line of raw.split("\n")) {
    ...
    // Strip surrounding quotes/brackets/punctuation (keep trailing ? which we
    // explicitly allow for interrogative titles).
    t = t.replace(/^[\s"'`《「『（(【\[]+|[\s"'`》」』）)】\]。．.！!]+$/g, "");
    if (!t) continue;
